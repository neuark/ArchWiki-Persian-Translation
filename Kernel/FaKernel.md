...
ما می توانیم حدس بزنیم که پینک در طول مراحل اولیه بارگزاری ماژول firewire_core  رخ داده است.(پس می‌توانیم فرض کنیم که سخت‌افزار فایر وایر دستگاه به دلیل خطای برنامه‌نویس با این نسخه از ماژول درایور فایر وایر سازگار نیست و باید منتظر انتشار نسخه جدید باشیم.) در حال حاضر، ساده ترین راه برای اجرای مجدد سیستم، جلوگیری از بارگذاری ماژول است. ما می توانیم این کار را در یکی از دو روش انجام دهیم:

* اگر ماژول در طول اجرای initramfs بارگزاری شود، با متغیر کرنل rd.blacklist=firewire_core راه اندازی مجدد می شود. 
* در غیر این صورت با ماژول متغیر کرنل module_blacklist=firewire_core راه اندازی مجدد کنید .

### راه اندازی مجدد به شل روت و رفع مشکل
شما دسترسی به شل روت نیاز دارید تا تغییراتی در سیستم ایجاد کنید تا پنیک دیگر رخ ندهد. اگر پنیک در فرایند بوت رخ دهد، استراتژی های متعددی برای دسترسی به شل روت قبل از قفل شدن دستگاه وجود دارد:
* راه اندازی مجدد سیستم با متغیر کرنل emergency, rd.emergency, یا -b برای دریافت درخواست برای ورود به سیستم درست پس از نصب فایل سیستم روت و systemd آغاز شده است.

> Note
در این مرحله، فایل سیستم روت در حالت فقط خواندن نصب خواهد شد. برای ایجاد تغییرات به عنوان کاربر روت mount -o remount,rw / را اجرا کنید .

راه اندازی مجدد با پارامتر هستهrescue، rd.rescue، single، s، S، یا 1 برای دریافت یک درخواست برای ورود به سیستم درست پس از مونت شدن فایل های محلی.
راه اندازی مجدد با متغیر کرنل systemd.debug_shell برای دسترسی به شل روت بسیار اولیه در tty9. با فشار دادن Ctrl +Alt + F9 به آن سوئیچ کنید.

> Note
> برای دیدن همه متغیر های کرنل ببینید  kernel-parameters.html

* به عنوان آخرین راه حل، با یک محیط نصب Arch Linux بوت کنید و قایل سیستم روت رو به /mnt مونت کنید و به عنوان یک کاربر روت اجرا کنید arch-chroot /mnt .
* سرویس یا برنامه ای را که باعث پنیک می شود غیرفعال کنید ، از بروزرسانی معیوب را بازیابی کنید یا یک مشکل پیکربندی را برطرف کنید.سرویس یا برنامه ای را که باعث پنیک می شود غیرفعال کنید ، از بروزرسانی معیوب را بازیابی کنید یا یک مشکل پیکربندی را برطرف کنید.

> Tip
> ممکن است لازم باشد در صورت خراب شدن initial ramdisk، یک image initial ramdisk جدید تولید شود.

### رگرسيون هاى عیب یابی

General troubleshooting#Debugging regressions را ببینید.

linux-mainline را امتحان کنید تا بررسی کنید که آیا این مسئله در حال حاضر ثابت است یا خیر. نظر پین شده همچنین مخزنی را ذکر می کند که حاوی کرنل های قبلا ساخته شده است، بنابراین ممکن است لازم نباشد که آن را به صورت دستی بسازید، که می تواند زمان ببرد.
همچنین ممکن است ارزش این را داشته باشد که کرنل LTS (لینوکس-LTS) را برای اشکال زدایی مسائلی که اخیرا ظاهر نشده اند، مورد بررسی قرار دهیم. نسخه های قدیمی تر کرنل LTS را می توان در آرشیو Arch Linux یافت.
